(function () {
  ...
  window.animevost_plugin = {
    ...
    stream: function (params, oncomplite, onerror) {
      ...
      this.AnimevostApiMethod(
        ...
        function (error) {
          console.error("Animevost Plugin: Ошибка получения плейлиста:", error);
          // Fallback: парсинг animevost.org
          Lampa.Network.get(
  ...
  if (typeof Lampa !== "undefined" && Lampa.Component && Lampa.Component.add) {
    ...
    // Добавление источника через StorageWorker, как в логах
    if (Lampa.Storage && Lampa.Storage.set) {
      console.log("Animevost Plugin: Добавление источника через StorageWorker");
